...
/*	Element* start=NULL; 
	bool bdry = false; // Randelement ?	
	unsigned int iv = 0;	
	bool more = false;
	do{ if(iv==e->nvert) error("no neighbor-elements");	//Durchlaeuft so lange bis das 1. Nachbarelement gefunden
		bdry = false;	
		edge = e->en[iv];
		 if(e->get_neighbor(iv)!=NULL) ne = e->get_neighbor(iv);
		else{ bdry = true; iv++;}
	}while(bdry==true);
	bdry = false;	
	if(start==NULL){ start = ne; //erstes Nachbarelement
		if(ne->is_triangle()==true)no_of_tris++;
		else if(ne->is_quad()==true)no_of_quads++; 
		else error("no quad, no triangle ?????");		
		for(int k=0; k< ne->nvert; k++){	
			for(int j = 0; j<e->nvert; j ++){
				if(ne->vn[k]==e->vn[j]){number_of_elem[j]--;} //gemeinsamer Knoten
			}	
		}	
	...
	//laeuft ccw um e bis start (=erstes Nachbarelement)
	
	do{	int i = 0;		
		if(bdry==true){	//falls Randelement erreicht, wieder zurueck zu e und ab naechsten Knoten
			iv= e->next_vert(iv); 			
			bdry = false;
			edge = e->en[iv];
			if(e->get_neighbor(iv)!=NULL) ne= e->get_neighbor(iv);
			else bdry = true;
		}else{	//ne wirklich Nachbar von e?			
			bool true_element = false;  //erstmal annehmen, dass es gar kein Nachbarelement ist
			in_list = false;		//ob es schon in der Liste vorkommt?
			if((ne!=start)&&(ne!=e)){
				for(unsigned int k = 0;k<ne_list->cnt; k++) //ueberpruefen ob schon in Liste, wenn ja, zurueck zu e
					if(ne->id== ne_list->idx[k]){ in_list = true; break;} //true_element = false!!!
				if(in_list ==false){
					for(int k=0; k< ne->nvert; k++){	//ueberpruefen ob wirklich Nachbar
						for(int j = 0; j<e->nvert; j ++){
							if(ne->vn[k]==e->vn[j]){ number_of_elem[j]--; true_element = true;}
						}	
					}
				}
				if(true_element == false){ bdry=true; break;}//keinen gemeinsamen Vertex mit e! =>falscher Nachbar=>zurueck zu e
				if(in_list ==false){
					if(ne->is_triangle()==true)no_of_tris++;
					else if(ne->is_quad()==true)no_of_quads++; 		
					ne_list->add_triplet(ne->id, no_of_nb, 1.0);
			...
			if((true_element == false)&&(ne!=start)&&(ne!=e)) break;//keinen gemeinsamen Vertex mit e! =>falscher Nachbar=>zurueck zu e	
			while(ne->en[i]!= edge){//richtigen Edge-index finden
				 i++;
				if(i>=ne->nvert) error("common edge not found");
			}
			i = ne->prev_vert(i); //vorheriger Knoten
			edge = ne->en[i];  //Kante zum naechsten Element
			if(ne->get_neighbor(i)!=NULL) ne = ne->get_neighbor(i);
			else bdry = true;
		}
	}while((ne!=start)||(bdry==true));	 
		
*/
	
	//for (unsigned int iv = 0; iv <ne_list->cnt; iv++) printf("idx:%i\n", ne_list->idx[iv]);
	...
	for(int i = 0; i<e->nvert; i++){			
		...
		for(int i = 0; i<e->nvert; i++){
			edge = e->en[i];
			if(e->get_neighbor(i)!=NULL){ 			
				ne= e->get_neighbor(i);
			}else if(e->get_neighbor(e->prev_vert(i))!=NULL){ 
					i=e->prev_vert(i); edge = e->en[i]; ne= e->get_neighbor(i);
					cw= false;
			}else error("angeblicher Nachbar nicht vorhanden, %i", number_of_elem[i]);			
				int k = 0;
				while(ne->en[k]!= edge){//richtigen Edge-index finden
					 k++;
					if(i>=ne->nvert) error("common edge not found");
				}
				if(cw = true) k = ne->next_vert(k); //cw laufen
				else k = ne->prev_vert(k);
				if(ne->get_neighbor(k)!=NULL) ne = ne->get_neighbor(k);
				else error("kein Nachbar vorhanden!!!, Vertex passt was nicht");			
				for(unsigned int l = 0;l<ne_list->cnt; l++) //ueberpruefen ob schon in Liste, wenn ja, zurueck zu e
					if(ne->id== ne_list->idx[l]) error("Nachbar schon in Liste");			
				for(unsigned int l=0; l< ne->nvert; l++){	
					for(int j = 0; j<e->nvert; j ++){
						if(ne->vn[l]==e->vn[l]) number_of_elem[l]--; 
					}	
				}
				if(ne->is_triangle()==true)no_of_tris++;
				else if(ne->is_quad()==true)no_of_quads++;			
	...
	//printf("size: ne_list %i, quads: %i, tris: %i\n", ne_list->cnt,no_of_quads,no_of_tris );
	...
	if(e->is_triangle()==true) tris[tri][3]= 2;
	else if(e->is_quad()==true) quads[qua][4]=2;
	if(e->is_triangle()==true) tri++;
	else if(e->is_quad()==true) qua++;	

	//Restlichen Nachbarn hinzufuegen
	bool xy_test = false;
	bool bdry_0 = true; //Knoten 0 noch suchen
	bool bdry_1 = false; //Knoten 1 noch suchen
	for(unsigned int j= 0; j<ne_list->cnt; j ++){
		ne = space.get_mesh()->get_element(ne_list->idx[j]);
		for (int iv = 0; iv < ne->nvert; iv++){ 			
			vn = ne->vn[iv];
			for(unsigned int k =0; k<ver; k ++){
				if((vn->x == verts[k][0])&&(vn->y == verts[k][1])){ //pruefen ob Knoten schon enthalten
					 xy_test = true; 
					if(ne->is_triangle()==true) tris[tri][iv]= k;
					else if(ne->is_quad()==true) quads[qua][iv]=k;
					if((k<e->nvert)&&(e->vn[k]->bnd!=1)){ bdry_1=false; break;}
					if(bdry_1==true){ mark[nm][1]= k; 
									nm++; bdry_1 = false; 			
									bdry_0=true;}					
					if(bdry_0==true){mark[nm][0]= k;mark[nm][2] = 2; 
						if(iv == ((ne->nvert)-1.0)){ 
						vn = ne->vn[0];				 		
						for(unsigned int k =0; k<ver; k ++){
							if((vn->x == verts[k][0])&&(vn->y == verts[k][1])){
								if((k<e->nvert)&&(e->vn[k]->bnd!=1)){ bdry_1=false; break;} 
 								if(bdry_1==true){ mark[nm][1]= k;nm++; }
							}
						}
						bdry_1 = false; bdry_0 = true;
						}else{ bdry_0 = false; bdry_1 = true;}
			...
			if(xy_test==false){	
				verts[ver][0] = vn->x;
				verts[ver][1] = vn->y;
				if(ne->is_triangle()==true) tris[tri][iv]= ver;
				else if(ne->is_quad()==true) quads[qua][iv]=ver;
				if(bdry_1==true){ mark[nm][1]= ver; 
							nm++; 
							}
				mark[nm][0]= ver;    //jeder hinzugefuegte Knoten ist nun ein Randknoten!!!
				mark[nm][2] = 2;   //keine Ahnung, einfach mal auf 2 gesetzt :-)
				bdry_0=false;
				bdry_1 = true;
				if(iv == ((ne->nvert)-1.0)){ 
					vn = ne->vn[0];
				 //d.h. alle Knoten wurden schon mal durchlaufen, deswegen anderen Eckknoten suchen
					for(unsigned int k =0; k<ver; k ++){
						if((vn->x == verts[k][0])&&(vn->y == verts[k][1])){
							if((k<e->nvert)&&(e->vn[k]->bnd!=1)){ bdry_1=false; break;} //wenn Knoten von e und kein Rand
 							if(bdry_1==true){ mark[nm][1]= k;nm++; }
						}
					}
					bdry_1 = false; bdry_0 = true;
		...
		if(ne->is_triangle()==true) tri++;
		else if(ne->is_quad()==true) qua++;
	...
	//H1ShapesetExtrapolation* shapeset_extrapolation = new H1ShapesetExtrapolation();
	H1Shapeset* shapeset_extrapolation = new H1Shapeset();
	Mesh* patch_mesh = new Mesh();
	patch_mesh->create(nv, verts, tri, tris, qua, quads,nm,mark );
	 H1Space* patch_space = new H1Space(patch_mesh, P_INIT, shapeset_extrapolation);
